      ...
      {!gameOver && !isCustomizing && (
        <div className="w-full flex justify-between items-end">
          ...
          <div className="w-auto md:w-1/4 flex flex-col items-end gap-2 pointer-events-auto z-20">
            ...
            <button onClick={onConnectSolana} disabled={!!solanaAddress} className={`px-3 py-1 border-2 border-black font-bold text-sm md:text-base shadow-[2px_2px_0_rgba(0,0,0,0.5)] ${solanaAddress ? 'bg-purple-400 text-black' : 'bg-purple-600 text-white hover:bg-purple-500 rotate-1'}`}>
              {solanaAddress ? `SOL: ${truncateAddress(solanaAddress)}` : 'CONNECT SOL'}
            </button>
          </div>
        </div>
      )}
